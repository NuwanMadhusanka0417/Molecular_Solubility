...
def bond_node_features_geognn(bond, pos):
    """
    Compute features for a single bond, normalized to [0, 1] per feature.

    Returns
    -------
    np.ndarray, shape [5]
        [bond_type_norm, is_conjugated, in_ring, bond_length_norm, stereo_norm]
    """
    bt = bond.GetBondType()
    bond_type = {
    ...
    bond_type_norm = bond_type / 4.0
    bond_length_norm = float(np.clip((length - 1.0) / 0.9, 0.0, 1.0))
    stereo_norm = stereo / 4.0

    return np.array([bond_type_norm,
                     is_conjugated,
                     in_ring,
                     bond_length_norm,
                     stereo_norm], dtype=np.float32)
...
def expand_atomic_features(data, mol):
    ...
    for i in range(data.edge_index.shape[1]):
        ...
        degrees[u] += 1
        degrees[v] += 1
    degrees = degrees / 2.0  # edge_index stores both directions; halve to get true degree
    ...
    for i, atomic_num in enumerate(atomic_numbers.squeeze(1).tolist()):
        ...
        valence_electrons[i] = _VALENCE_DICT.get(int(atomic_num))

        # Hybridization one-hot from RDKit (actual bonding environment)
        hybridization[i, :] = torch.tensor(
            _HYBRID_MAP.get(atom.GetHybridization(), [0, 0, 0]),
            dtype=torch.float32,
        )
...
def create_graph_list(dataset):
  ...
  for data in dataset:
    ...
    # node features; bond info is only in edge_attr (not duplicated into node features)
