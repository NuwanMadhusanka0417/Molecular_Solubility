...
for dim in dims:
    train_graphs = create_graph_list(train_data)
    test_graphs = create_graph_list(test_data)
    model_eq1 = GraphCNN(train_graphs[0].node_features.shape[1], num_layers, delta_eq1, graph_pooling_type, neighbor_pooling_type, device, equation_eq1) #.to(device)
    
    # print("GVFA main: edge_attr.shape ", test_graphs[0].edge_features.shape)
    # torch.Size([8])
    ts_graph = test_graphs.copy()
    tr_graph = train_graphs.copy()
    test_Gs, W = VSA_conversion(ts_graph, dim)
    train_Gs,_ = VSA_conversion(tr_graph, dim)

    train_embeddings_eq1, train_labels_eq1 = getEmbedding(model_eq1, device, train_Gs)
    test_embeddings_eq1, test_labels_eq1 = getEmbedding(model_eq1, device, test_Gs)

    train_embeddings_eq1 = train_embeddings_eq1.squeeze(0)

    test_embeddings_eq1 = test_embeddings_eq1.squeeze(0)

    train_embeddings_eq1 = train_embeddings_eq1.detach().cpu().numpy().astype('float32')
    test_embeddings_eq1  = test_embeddings_eq1.detach().cpu().numpy().astype('float32')
