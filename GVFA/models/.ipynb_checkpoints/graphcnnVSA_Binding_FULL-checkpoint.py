...
class GraphCNN(nn.Module):
    def __init__(self, input_dim, num_layers, delta, graph_pooling_type, neighbor_pooling_type, device):
        '''
...
#         return Adj_block.to(self.device)
    ...
    def invert_permutation(self, perm):
        ...
        for i, p in enumerate(perm):
            inverse[p] = i
        return inverse
    def next_layer_eps(self, h, layer, padded_neighbor_list = None, Adj_block = None, delta = 1):
        
        n_rows, n_cols = h.shape

#         # Calculate the effective rotation by taking modulo 'n_cols'
#         k = (1) % n_cols

#         # Create indices for selecting the elements after rotation
#         indices = (torch.arange(n_cols) - k) % n_cols
#         # Use advanced indexing to select and rearrange the elements in each row
#         rotated_matrix = h[:, indices]
        
    
    
        
        torch.manual_seed(0) #0
        permutation = torch.randperm(n_cols)

# #         # rearrange the columns based on the permutation
        # rotated_matrix = h[:, permutation]

        
        
        
        # # Convert the permutation to a permutation matrix
        # perm_matrix = self.permutation_to_matrix(permutation).to(h.device)


        # # Compute the effective permutation matrix after applying it 'layer' times
        # effective_perm_matrix = torch.matrix_power(perm_matrix, layer+1) ### layer+1

        # # Apply the effective permutation
        # rotated_matrix = torch.mm(h, effective_perm_matrix)


        # Apply permutation directly without converting to a matrix
...
#             h = self.next_layer_eps(h, layer, Adj_block=Adj_block, delta=self.delta)
#             hidden_rep.append(h)

#         # Optionally, return the embeddings from the last layer for downstream tasks
#         # if return_embedding:
#         #     return hidden_rep[-1]
#         return hidden_rep
